...
module "mender_eks_role" {
  ...
  oidc_providers = {
    default = {
      ...
      namespace_service_accounts = ["default:kube-system", "default:mender-sa", "mender:mender-sa"]
    }
  }
  ...
  depends_on = [module.mender_eks] 
}

module "mender_sa_role" {
  source    = "terraform-aws-modules/iam/aws//modules/iam-role-for-service-accounts-eks"
  allow_self_assume_role = true
  ...
  role_policy_arns = {
    mender_sa           = aws_iam_policy.mender_sa.arn
  }
...
resource "aws_iam_policy" "mender_sa" {
...
resource "aws_iam_role" "mender_eks_node_role" {
...
resource "aws_iam_role_policy_attachment" "node_eks_worker" {
  ...
  role       = aws_iam_role.mender_eks_node_role.name
}
